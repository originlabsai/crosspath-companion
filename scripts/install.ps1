# install.ps1 - CrossPath Companion Installer for Windows
# Usage: irm https://get.crosspath.im/mcp.ps1 | iex
#        irm https://get.crosspath.im/mcp.ps1 | iex -args install|update|uninstall
...
$InstallDir = if ($env:MCP_CLIENT_INSTALL_DIR) { $env:MCP_CLIENT_INSTALL_DIR } else { "$env:LOCALAPPDATA\CrossPath\bin" }
$Repo = "originlabsai/crosspath-companion"
...
function Write-Banner {
    Write-Host ""
    Write-Host "CrossPath Companion Installer" -ForegroundColor White
    Write-Host ""
}
...
function Verify-Checksum {
    ...
    try {
        ...
        $expectedLine = ($checksums.Content -split "`n" | Where-Object { $_ -match "cross_companion-windows-amd64" }) -split '\s+' | Select-Object -First 1
...
function Install-MCPClient {
    ...
    $url = "https://github.com/$Repo/releases/download/v$Version/cross_companion-windows-amd64.exe"

    Write-Info "Installing cross_companion v$Version to $InstallDir"
    ...
    # Download
    $tempFile = "$env:TEMP\cross_companion-download.exe"
    ...
    Move-Item -Force $tempFile "$InstallDir\cross_companion.exe"
    ...
    if ($userPath -notlike "*$InstallDir*") {
        ...
        Write-Host ""
        Write-Host "  Restart your terminal to use cross_companion" -ForegroundColor Yellow
        Write-Host ""
    }

    Write-Success "cross_companion v$Version installed!"
    ...
    Write-Host "    cross_companion login     # Authenticate with CrossPath"
    Write-Host ""
    ...
    Write-Host "    cross_companion start     # Start manually"
    Write-Host "    cross_companion status    # Check connection"
    Write-Host "    cross_companion --help    # See all commands"
    Write-Host ""
    Write-Host "  Documentation: https://docs.crosspath.im/cross_companion"
    Write-Host ""
}
...
function Update-MCPClient {
    ...
    $current = Get-Command cross_companion -ErrorAction SilentlyContinue
    if (-not $current) {
        Write-Info "cross_companion not found, installing..."
        Install-MCPClient
        return
    }
    ...
    # Download to temp
    $tempFile = "$env:TEMP\cross_companion-update.exe"
    $url = "https://github.com/$Repo/releases/download/v$Version/cross_companion-windows-amd64.exe"
    ...
    Write-Success "cross_companion updated to v$Version!"
    Write-Host ""
    Write-Host "  Run 'cross_companion --version' to verify"
    Write-Host ""
}
...
function Uninstall-MCPClient {
    ...
    $current = Get-Command cross_companion -ErrorAction SilentlyContinue
    if (-not $current) {
        Write-Error-Custom "cross_companion not found"
    }
    ...
    $configDir = "$env:USERPROFILE\.crosspath"
    if ($RemoveConfig) {
    ...
    Write-Success "cross_companion uninstalled"
}

# Show help
function Show-Help {
    Write-Host @"
CrossPath Companion Installer for Windows

Usage:
  irm https://get.crosspath.im/mcp.ps1 | iex
  irm https://get.crosspath.im/mcp.ps1 | iex -args install|update|uninstall
...
Examples:
  ...
  `$env:MCP_CLIENT_VERSION = "1.2.0"; irm https://get.crosspath.im/mcp.ps1 | iex

  # Install to custom directory
  `$env:MCP_CLIENT_INSTALL_DIR = "C:\bin"; irm https://get.crosspath.im/mcp.ps1 | iex

  # Update
  irm https://get.crosspath.im/mcp.ps1 | iex -args update

  # Uninstall with config removal
  `$env:REMOVE_CONFIG = "true"; irm https://get.crosspath.im/mcp.ps1 | iex -args uninstall
