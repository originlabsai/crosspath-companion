...
var DevicesCmd = &cobra.Command{
	...
	Long:  `List, rename, or revoke devices connected to your CrossPath account.`,
}
...
func runDevicesList(cmd *cobra.Command, args []string) error {
	...
	if cfg.AuthToken == "" {
		return fmt.Errorf("not logged in. Run: cross_companion login")
	}
	...
	if resp.StatusCode == http.StatusUnauthorized {
		...
		if cfg.Device != nil && cfg.Device.RefreshToken != "" {
			if err := RefreshDeviceToken(cfg); err != nil {
				return fmt.Errorf("session expired. Please run: cross_companion login")
			}
	...
	fmt.Println("─────────────────────────────────────────────────────────────")
	fmt.Println("Commands: cross_companion devices revoke <id> | cross_companion devices rename <id> <name>")
