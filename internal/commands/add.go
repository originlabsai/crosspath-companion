...
enabled by default and started when you run 'cross_companion start'.

Examples:
  cross_companion add filesystem --path /usr/local/bin/mcp-server-filesystem
  cross_companion add database --path ./mcp-server-sqlite --type stdio`,
...
func runAdd(cmd *cobra.Command, args []string) error {
	...
	fmt.Println()
	fmt.Println("Server will be started automatically when you run 'cross_companion start'")
