...
var StartCmd = &cobra.Command{
	...
	Long: `Starts the MCP client daemon, connects to the CrossPath backend,
...
func runStart(cmd *cobra.Command, args []string) error {
	...
	if cfg.AuthToken == "" {
		return fmt.Errorf("not authenticated. Please run 'cross_companion login' first")
	}

	verbose, _ := cmd.Flags().GetBool("verbose")

	log.Println("🚀 Starting CrossPath Companion")
	...
	if auth.IsTokenExpiringSoon(cfg.TokenExpiry, auth.TokenExpiryBuffer) {
		...
		// Use device token refresh if we have device credentials (from cross_companion login)
		...
		if refreshErr != nil {
			if errors.Is(refreshErr, auth.ErrTokenExpired) {
				return fmt.Errorf("token expired and no refresh token available. Please run 'cross_companion login'")
			}
			return fmt.Errorf("token refresh failed: %w. Please run 'cross_companion login'", refreshErr)
		}
	...
	if len(enabledServers) == 0 {
		log.Println("⚠️  No MCP servers configured. Add servers with 'cross_companion add'")
	}
