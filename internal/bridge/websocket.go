...
func (b *Bridge) ConnectWithRetry() {
	...
	for {
		...
		b.mutex.RUnlock()
		if authFailed {
			log.Println("❌ Authentication failed. Please run: cross_companion login")
			return
		}
		...
		if errors.Is(err, ErrAuthenticationFailed) {
			...
			b.authFailed = true
			b.mutex.Unlock()
			log.Println("❌ Token expired and refresh failed. Please run: cross_companion login")
			return
		}
...
// SendFileAccessEvent sends a file access event to the backend for transparency logging.
// operation is one of: "read", "write", "delete", "list", "exec", "http"
// filePath is the local path accessed (empty for non-file operations).
// sessionID links the event to the triggering chat session.
func (b *Bridge) SendFileAccessEvent(operation, filePath, toolName, sessionID string, success bool, errorMsg string) {
	// Non-blocking: if the write channel is full, drop the event rather than stalling tool execution
	msg := Message{
