...
func (w *WizardModel) View() string {
	...
	// Small logo
	for _, line := range strings.Split(crosspathLogoSmall, "\n") {
		if line == "" {
			continue
		}
	...
	b.WriteString("\n")
	b.WriteString(centerText(titleStyle.Render("CrossPath Companion Setup"), width))
	b.WriteString("\n")
	...
	if w.chromeBinary == "" {
		...
		b.WriteString("\n")
		b.WriteString(lipgloss.NewStyle().Padding(0, 2).Render(
			theme.ValueMuted.Render("Choose which Chrome profile CrossPath Companion should use for browsing:"),
		))
		b.WriteString("\n\n")
...
func (w *WizardModel) renderChromeNotFound(theme *Theme, width int) string {
	...
	content.WriteString("\n\n")
	content.WriteString(theme.Value.Render("  Google Chrome is required for CrossPath Companion's browser tools."))
	content.WriteString("\n\n")
	content.WriteString(theme.Label.Render("  To install Chrome:"))
	content.WriteString("\n")
	content.WriteString(theme.ValueMuted.Render("  Windows: "))
	...
	content.WriteString("\n\n")
	content.WriteString(theme.Help.Render("  Install Chrome and re-run CrossPath Companion."))
	content.WriteString("\n\n")
