...
func NewSettings(state *tui.AppState) *Settings {
	...
	if state.LatestVersion != "" {
		...
		versionDesc = "Run: curl -fsSL https://github.com/originlabsai/crosspath-companion | bash"
	}
	...
	return &Settings{
		...
		items: []SettingItem{
			...
			},
			{
				Key:         "config_path",
				Label:       "Config Path",
				Value:       "~/.crosspath/mcp-config.yaml",
			...
			},
			{
				Key:         "log_path",
				Label:       "Log Path",
				Value:       "~/.crosspath/logs/cross_companion.log",
