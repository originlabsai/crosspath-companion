...
func (a *App) viewHeader(w int) string {
	...
	logo := a.theme.LogoDot.Render("◉") + a.theme.Logo.Render(" CrossPath Companion")
...
func (a *App) viewSettings(w int) string {
	...
	rows := []string{
		...
		a.theme.Label.Render("Config:") + "\n  " + a.theme.Value.Render("~/.crosspath/mcp-config.yaml"),
...
func (a *App) overlayQuitDialog(base string, w, h int) string {
	...
	title := a.theme.ModalTitle.Render("Exit CrossPath Companion")
	content.WriteString(title)
	content.WriteString("\n\n")
