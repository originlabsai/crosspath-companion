...
func GetSocketPath() string {
	if runtime.GOOS == "windows" {
		return filepath.Join(os.TempDir(), "cross_companion.port")
	}
	runtimeDir := os.Getenv("XDG_RUNTIME_DIR")
	if runtimeDir == "" {
		runtimeDir = os.TempDir()
	}
	return filepath.Join(runtimeDir, "cross_companion.sock")
}
...
func GetPIDPath() string {
	...
	return filepath.Join(runtimeDir, "cross_companion.pid")
}
...
func (d *Daemon) setupBridgeHandlers() {
	d.bridge.SetToolCallHandler(func(tc bridge.ToolCall) {
		...
		if err != nil {
			...
			d.bridge.SendToolResult(tc.CallID, false, "", errMsg)
		} else {
		...
		// Stream file access event to CrossPath backend for transparency log
		op, filePath := classifyToolAccess(tc.ToolName, tc.Arguments)
		if op != "" {
			d.bridge.SendFileAccessEvent(op, filePath, tc.ToolName, "", success, errMsg)
		}
	})
...
// classifyToolAccess maps a tool name to a file access operation and path.
// Returns ("", "") for tools that don't touch the local filesystem.
func classifyToolAccess(toolName string, args map[string]interface{}) (operation, filePath string) {
	pathArg := func() string {
		for _, key := range []string{"path", "file_path", "filepath", "directory", "dir"} {
	...
	case "grep":
		return "read", pathArg()
	case "execute_bash", "run_background":
